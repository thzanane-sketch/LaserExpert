...
module.exports = async function handler(req, res) {
  ...
  const allowedOrigins = [
    'https://test-laser.design.webflow.com',
    'https://laser-expert.vercel.app',
    'http://localhost:3000',
    'null' // pour les fichiers ouverts en local pendant les tests
  ];
  const origin = req.headers.origin;
  if (allowedOrigins.includes(origin)) {
    res.setHeader('Access-Control-Allow-Origin', origin);
  } else {
    res.setHeader('Access-Control-Allow-Origin', 'https://test-laser.design.webflow.com');
  }
